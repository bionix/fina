...
echo -n "# Generated by Fina v${VERSION} on " >> "$TMPDIR/rules.new"
date -Is >> "$TMPDIR/rules.new"
for RULEFILE in $RULEFILES; do
    echo "# BEGIN of file '$RULEFILE'" >> "$TMPDIR/rules.new"
    cat "$RULEFILE" >> "$TMPDIR/rules.new"
    echo "# END of file '$RULEFILE'" >> "$TMPDIR/rules.new"
done
echo -n "# Generation complete on " >> "$TMPDIR/rules.new"
date -Is >> "$TMPDIR/rules.new"
